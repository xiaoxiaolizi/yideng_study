...
(function() {
    ...
    // underscore 对象 _ 将会依托于当前的所处环境，挂载到不同的全局空间当中（浏览器的全局对象是 window，node 的全局对象是 global）。
    // 下面代码展示了 underscore 是如何判断自己所处环境的，这个判断逻辑也为我们自己想要撰写前后端通用的库的时候提供了帮助：
    var root = typeof self == 'object' && self.self === self && self ||
              typeof global == 'object' && global.global === global && global ||
              this ||
              {};
    // 在 underscore 的判断所处环境的代码中，似乎我们没有看到 window 对象的引用，
    // 其实，在浏览器环境下，self 保存的就是当前 window 对象的引用。那么相比较于使用 window，使用 self 有什么优势呢？
    // 概括来说，就是 self 还能用于一些不具有窗口的上下文环境中，比如 Web Workers。所以，为了服务于更多场景，underscore 选择了更加通用的 self 对象
  
    // Save the previous value of the `_` variable.
    // 默认情况下，underscore 对象 _ 会覆盖全局对象上同名的 _ 属性。
    // 但是，underscore 也不过于蛮横，他会保存之前已经存在的 _ 属性, 因为像是 lodash 这样的一些库也喜欢将自己的对象命名为 _：
